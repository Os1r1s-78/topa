...
#include "ui_ztopacontroldialog.h"
...
std::set<std::string> ZTopaControlDialog::setSelectedMints;
std::set<CMintMeta> ZTopaControlDialog::setMints;

ZTopaControlDialog::ZTopaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTopaControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZTopaControlDialog::~ZTopaControlDialog()
{
    delete ui;
}

void ZTopaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTopaControlDialog::updateList()
{
...
void ZTopaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTopaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZTopa_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTopaControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZTopaControlDialog::GetSelectedMints()
{
...
void ZTopaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
