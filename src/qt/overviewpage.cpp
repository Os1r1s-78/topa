...
// Copyright (c) 2017-2019 TopaChain LLC
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::TOPA)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sTOPAPercentage, QString& szTOPAPercentage)
{
    int nPrecision = 2;
    ...
    szTOPAPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sTOPAPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount topaAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = topaAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // TOPA labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, topaAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelTOPAPercent->setText(sPercentage);
    ui->labelzTOPAPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zTOPA.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in topachain.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in topachain.conf");
    }
    ...
    bool showTOPAAvailable = settingShowAllBalances || topaAvailableBalance != nTotalBalance;
    bool showWatchOnlyTOPAAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showTOPAPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyTOPAPending = watchUnconfBalance != 0;
    bool showTOPALocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyTOPALocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showTOPAAvailable || showWatchOnlyTOPAAvailable);
    ui->labelBalanceText->setVisible(showTOPAAvailable || showWatchOnlyTOPAAvailable);
    ui->labelWatchAvailable->setVisible(showTOPAAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showTOPAPending || showWatchOnlyTOPAPending);
    ui->labelPendingText->setVisible(showTOPAPending || showWatchOnlyTOPAPending);
    ui->labelWatchPending->setVisible(showTOPAPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showTOPALocked || showWatchOnlyTOPALocked);
    ui->labelLockedBalanceText->setVisible(showTOPALocked || showWatchOnlyTOPALocked);
    ui->labelWatchLocked->setVisible(showTOPALocked && showWatchOnly);
    ...
    bool showzTOPAAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzTOPAUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzTOPAImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzTOPAAvailable);
    ui->labelzBalanceMatureText->setVisible(showzTOPAAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzTOPAUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzTOPAUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzTOPAImmature);
    ui->labelzBalanceImmatureText->setVisible(showzTOPAImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelTOPAPercent->setVisible(showPercentages);
    ui->labelzTOPAPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("TOPA")
    updateDisplayUnit();
}
