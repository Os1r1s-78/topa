...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TOPA);
    ...
    if (!settings.contains("nAnonymizeTopaChainAmount"))
        settings.setValue("nAnonymizeTopaChainAmount", 1000);

    nAnonymizeTopaChainAmount = settings.value("nAnonymizeTopaChainAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTopaChainAmount"))
        SoftSetArg("-anonymizetopachainamount", settings.value("nAnonymizeTopaChainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in topachain.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTopaChainAmount:
            return QVariant(nAnonymizeTopaChainAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTopaChainAmount:
            nAnonymizeTopaChainAmount = value.toInt();
            settings.setValue("nAnonymizeTopaChainAmount", nAnonymizeTopaChainAmount);
            emit anonymizeTopaChainAmountChanged(nAnonymizeTopaChainAmount);
            break;
        case CoinControlFeatures:
