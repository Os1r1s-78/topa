                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zTOPApayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zTOPA ought to be enough for anybody." - Bill Gates, 2017
    ui->zTOPApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTOPASyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTOPApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTOPA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTOPA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTOPA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTOPA in ") +
...
void PrivacyDialog::on_pushButtonSpendzTOPA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTOPA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTOPA
        sendzTOPA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTOPA
    sendzTOPA();
}

void PrivacyDialog::on_pushButtonZTopaControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZTopaControlDialog* zTopaControl = new ZTopaControlDialog(this);
    zTopaControl->setModel(walletModel);
    zTopaControl->exec();
}

void PrivacyDialog::setZTopaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTOPASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTOPA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TopaChain Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTOPApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTOPA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TOPA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTOPA</b>";
    ...
    // use mints from zTOPA selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTopaControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZTopaControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zTOPA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zTOPA"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZTOPA_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zTOPA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zTOPA"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zTOPA was spent successfully update the addressbook with the label
    ...
    // Clear ztopa selector in case it was used
    ZTopaControlDialog::setSelectedMints.clear();
    ui->labelzTOPASelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTOPA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Topa, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTOPA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->ztopaTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTOPA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTOPA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTOPA "));
    ui->labelzTOPAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTOPASyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzTOPA->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zTOPA
        ui->pushButtonMintzTOPA->setEnabled(false);
        ui->pushButtonMintzTOPA->setToolTip(tr("zTOPA is currently disabled due to maintenance."));

        // Spend zTOPA
        ui->pushButtonSpendzTOPA->setEnabled(false);
        ui->pushButtonSpendzTOPA->setToolTip(tr("zTOPA is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zTOPA
        ui->pushButtonMintzTOPA->setEnabled(true);
        ui->pushButtonMintzTOPA->setToolTip(tr("PrivacyDialog", "Enter an amount of TOPA to convert to zTOPA", 0));

        // Spend zTOPA
        ui->pushButtonSpendzTOPA->setEnabled(true);
        ui->pushButtonSpendzTOPA->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}