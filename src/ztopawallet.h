...
#ifndef TOPACHAIN_ZTOPAWALLET_H
#define TOPACHAIN_ZTOPAWALLET_H
...
public:
    CzTOPAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZTOPA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZTOPA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //TOPACHAIN_ZTOPAWALLET_H
