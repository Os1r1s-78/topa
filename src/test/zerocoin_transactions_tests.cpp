...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.ztopaTracker = unique_ptr<CzTOPATracker>(new CzTOPATracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZTOPA_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZTOPA_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
