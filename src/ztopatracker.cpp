...
CzTOPATracker::CzTOPATracker(std::string strWalletFile)
{
...
CzTOPATracker::~CzTOPATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTOPATracker::Init()
{
...
bool CzTOPATracker::Archive(CMintMeta& meta)
{
...
bool CzTOPATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTOPATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTOPATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTOPATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzTOPATracker::GetSerialHashes()
{
...
CAmount CzTOPATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTOPATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzTOPATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzTOPATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTOPATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTOPATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTOPATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTOPATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTOPATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTOPATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzTOPATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTOPATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTOPATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTOPATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTOPATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTOPATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTOPATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dztopa from DB\n", __func__, listDeterministicDB.size());
    }
