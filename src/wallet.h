...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TOPA at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTOPA send
enum ZerocoinSpendStatus {
    ZTOPA_SPEND_OKAY = 0,                            // No error
    ZTOPA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTOPA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTOPA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTOPA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTOPA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTOPA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTOPA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTOPA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTOPA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTOPA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTOPA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTOPA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTOPA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTOPA_SPENT_USED_ZTOPA = 14,                      // Coin has already been spend
    ZTOPA_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZTOPA_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZTopaBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZTOPAOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fztopaAuto) const;
    ...
    */
    ...
    CzTOPAWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzTOPATracker> ztopaTracker;
    ...
    void setZWallet(CzTOPAWallet* zwallet)
    {
        zwalletMain = zwallet;
        ztopaTracker = std::unique_ptr<CzTOPATracker>(new CzTOPATracker(strWalletFile));
    }

    CzTOPAWallet* getZWallet() { return zwalletMain; }
    ...
    void setZTopaAutoBackups(bool fEnabled)
    {
