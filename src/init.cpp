...
#include "config/topachain-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("topachain-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "topachain.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "topachaind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the TOPA and zTOPA money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 25000, 25000));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in TOPA/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in TOPA/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, topachain, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in TOPA/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all TopaChain specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupztopa=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTopa minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ztopabackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zTOPA backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizetopachainamount=<n>     " + strprintf(_("Keep N TOPA anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i TopaChain LLC"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("topachain-loadblk");
...
/** Sanity checks
 *  Ensure that TopaChain is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TopaChain Core is shutting down."));
...
#endif
    // Make sure only a single TopaChain process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TopaChain Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("TopaChain version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //TopaChain specific: zerocoin and spork DB's
                ...
                // TopaChain: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZTOPAMinted();
                        RecalculateZTOPASpent();
                    }
                    RecalculateTOPASupply(1);
                }
                ...
                // TopaChain: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of TopaChain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart TopaChain Core to complete") << "\n";
        ...
        zwalletMain = new CzTOPAWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zTOPA wallet..."));

        bool fEnableZTopaBackups = GetBoolArg("-backupztopa", true);
        pwalletMain->setZTopaAutoBackups(fEnableZTopaBackups);
...
//    }
//
//    nAnonymizeTopaChainAmount = GetArg("-anonymizetopachainamount", 0);
//    if (nAnonymizeTopaChainAmount > 999999) nAnonymizeTopaChainAmount = 999999;
//    if (nAnonymizeTopaChainAmount < 2) nAnonymizeTopaChainAmount = 2;
    ...
    LogPrintf("Anonymize TopaChain Amount %d\n", nAnonymizeTopaChainAmount);
    ...
    /* Denominations
       ...
       For example:
       1TOPA+1000 == (.1TOPA+100)*10
       10TOPA+10000 == (1TOPA+1000)*10
    */
