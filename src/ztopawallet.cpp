...
CzTOPAWallet::CzTOPAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztopa seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTOPASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTOPASeed_deprecated()) {
                    LogPrintf("%s: Updated zTOPA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztopa seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztopa wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTOPAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTOPACount(nCountLastUsed);
    else if (!walletdb.ReadZTOPACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTOPAWallet::Lock()
{
    seedMaster = 0;
}

void CzTOPAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTOPAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTOPA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTOPAWallet::LoadMintPoolFromDB()
{
...
void CzTOPAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTOPAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTOPAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztopaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTOPAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTOPA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztopaTracker which also adds to database
    pwalletMain->ztopaTracker->Add(dMint, true);
...
void CzTOPAWallet::SeedToZTOPA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTOPAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTOPAWallet::GenerateDeterministicZTOPA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTOPAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTOPAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
