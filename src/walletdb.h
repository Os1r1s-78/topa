...
public:
    ...
    bool WriteZTOPASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZTOPASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZTOPASeed_deprecated(uint256& seed);
    bool EraseZTOPASeed();
    bool EraseZTOPASeed_deprecated();

    bool WriteZTOPACount(const uint32_t& nCount);
    bool ReadZTOPACount(uint32_t& nCount);
