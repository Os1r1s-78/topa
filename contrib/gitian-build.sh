...
VERSION=
commit=false
url=https://github.com/TopaChainOSS/TOPA-Core
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the topachain, gitian-builder, gitian.sigs, and topachain-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/TopaChainOSS/TOPA-Core
...
then
	# Make output folder
	mkdir -p "./topachain-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit topachain=${COMMIT} --url topachain=${url} ../topachain/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/topachain-*.tar.gz build/out/src/topachain-*.tar.gz ../topachain-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit topachain=${COMMIT} --url topachain=${url} ../topachain/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/topachain-*.zip build/out/topachain-*.exe ../topachain-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit topachain=${COMMIT} --url topachain=${url} ../topachain/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/topachain-*.tar.gz build/out/topachain-*.dmg ../topachain-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit topachain=${COMMIT} --url topachain=${url} ../topachain/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../topachain/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/topachain-*.tar.gz build/out/src/topachain-*.tar.gz ../topachain-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../topachain/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/topachain-*win64-setup.exe ../topachain-binaries/${VERSION}
	    mv build/out/topachain-*win32-setup.exe ../topachain-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../topachain/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/topachain-osx-signed.dmg ../topachain-binaries/${VERSION}/topachain-${VERSION}-osx.dmg
	fi
	popd || exit
